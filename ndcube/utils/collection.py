...
def _sanitize_user_aligned_axes(data, aligned_axes):
    ...
    *. A single int if only one axis is aligned and if the aligned axis in each cube
    ...
    if isinstance(data[0].dimensions, tuple):
        cube0_dims = np.asarray([d.to_value('pix') for d in data[0].dimensions])
        cube0_dims = np.asarray([cube0_dims[i] for i in aligned_axes[0]])
    else:
        cube0_dims = data[0].dimensions[np.asarray(aligned_axes[0])].to_value('pix')
    ...
    # all elements of each sub-tuple are ints.
        ...
        for i in range(n_cubes):
            ...
            for j, axis in enumerate(aligned_axes[i]):
                ...
                cube_lengths_equal[j] = data[i].dimensions[axis].value == cube0_dims[j]
...
def _update_aligned_axes(drop_aligned_axes_indices, aligned_axes, first_key):
    ...
    if len(drop_aligned_axes_indices) <= 0:
        ...
        for key in aligned_axes.keys():
            cube_aligned_axes = np.asarray(aligned_axes[key])
...
def assert_aligned_axes_compatible(data_dimensions1, data_dimensions2, data_axes1, data_axes2):
    ...
    if data_axes1 is not None and data_axes2 is not None:  # aligned_axes are being used for both collections
        ...
        if not all(data_dimensions1[np.asarray(data_axes1)] == data_dimensions2[np.asarray(data_axes2)]):
